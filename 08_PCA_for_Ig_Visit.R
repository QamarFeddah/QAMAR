...
dir.create("outputs/Ig_Visit_PCA", showWarnings = FALSE, recursive = TRUE)
...
for (titer in titer_vars) {       #loop over every antibody
  ...
  p1 <- ggplot(
    pca_df,
    aes(x = PC1, y = PC2, color = .data[[grp_name]], shape = Visit)
  ) +
    geom_point(size = 3) +
    theme_minimal() +
    labs(
  ...
  p2 <- ggplot(
    pca_df,
    aes(x = PC3, y = PC4, color = .data[[grp_name]], shape = Visit)
  ) +
    geom_point(size = 3) +
    theme_minimal() +
    labs(
  ...
  ## Save plots
  pdf(
    file = paste0("outputs/Ig_Visit_PCA/PCA_", titer, "_groups.pdf"),
    width = 9,
    height = 7
  )
  print(p1)
