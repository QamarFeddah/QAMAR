...
dir.create("outputs/DESeq_Sex/")
## Create a DESeq Data Set
dds <- DESeqDataSetFromMatrix(countData = count_matrix, colData = colData, design = ~ Sex)
...
subset(res, padj < 0.1 & (log2FoldChange > 0.5 | log2FoldChange < -0.5)) %>%
  as.data.frame() %>%
  write.csv(file.path("outputs/DESeq_Sex/UP_Down_regulated_Genes.csv"))
...
vlcn_plot <-
  EnhancedVolcano(res_dataframe,
                  ...
                  x = "log2FoldChange",
                  y = "padj",
                  title = "Gene expression associated with Sex, baseline male",
                  pCutoff = 0.1,
                  FCcutoff = 0.5,
                  pointSize = 2.0,
                  labSize = 5.0)

pdf(file = "outputs/DESeq_Sex/ConM_Sex_DEG_Volcano.pdf", width = 12, height = 9)
print(vlcn_plot)
dev.off()
