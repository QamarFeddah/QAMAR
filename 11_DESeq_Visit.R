...
subset(res, padj < 0.1 & (log2FoldChange > 0.5 | log2FoldChange < -0.5)) %>%
  as.data.frame() %>%
  write.csv(file.path("outputs/DESeq_visit/UP_Down_regulated_Genes.csv"))
...
vlcn_plot <-
  EnhancedVolcano(res_dataframe,
                  ...
                  x = "log2FoldChange",
                  y = "padj",
                  title = "Gene expression associated with Visit, baseline V18",
                  pCutoff = 0.1,
                  FCcutoff = 0.5,
                  pointSize = 2.0,
                  labSize = 5.0)

pdf(file = "outputs/DESeq_visit/ConM_Visit_DEG_Volcano.pdf", width = 12, height = 9)
print(vlcn_plot)
dev.off()
