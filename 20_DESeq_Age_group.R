...
dir.create("outputs/DESeq_Age_group/")
## Create a DESeq Data Set
dds <- DESeqDataSetFromMatrix(countData = count_matrix, colData = colData, design = ~ Age_group)

## Remove genes that have counts less than 10 in total and set the base condition
keep_genes <- rowSums(counts(dds)) >= 10
dds <- dds[keep_genes,]
dds$Age_group <- factor(dds$Age_group)
dds$Age_group <- relevel(dds$Age_group, ref = "1")
...
subset(res, padj < 0.1 & (log2FoldChange > 0.5 | log2FoldChange < -0.5)) %>%
  as.data.frame() %>%
  write.csv(file.path("outputs/DESeq_Age_group/UP_Down_regulated_Genes.csv"))
...
vlcn_plot <-
  EnhancedVolcano(res_dataframe,
                  ...
                  x = "log2FoldChange",
                  y = "padj",
                  title = "Gene expression associated with Age_group",
...
pdf(file = "outputs/DESeq_Age_group/ConM_Age_group_DEG_Volcano.pdf", width = 12, height = 9)
print(vlcn_plot)
dev.off()
